...
from thirdparty.kplanes.models.density_fields import KPlaneDensityField
from thirdparty.kplanes.models.kplane_field import KPlaneField
from nerf_kplanes.network import KPlaneRBFField
from thirdparty.kplanes.ops.activations import init_density_activation
from thirdparty.kplanes.raymarching.ray_samplers import (
    UniformLinDispPiecewiseSampler, UniformSampler,
    ProposalNetworkSampler, RayBundle, RaySamples
)
from thirdparty.kplanes.raymarching.spatial_distortions import SceneContraction, SpatialDistortion
from thirdparty.kplanes.utils.timer import CudaTimer
...
class LowrankModel(nn.Module):
    def __init__(self,
        ...
        elif self.field_type == 'kplane_rbf':
            self.field = KPlaneRBFField(
                aabb,
                grid_config=self.config,
                concat_features_across_scales=self.concat_features_across_scales,
                multiscale_res=self.multiscale_res,
        ...
        # Count parameters
        self.n_params = sum(np.prod(p.shape) for p in self.parameters())
        if self.field_type == 'kplane_rbf' and self.field.rbf_type.endswith('_a'):
            in_dim = self.field.in_dim
            self.n_params -= self.field.n_kernel * int((in_dim-1)*in_dim/2)
    ...
    def step_after_iter(self, step):
        ...
        return self.field.step_after_iter(step)
    ...
    def get_params(self, lr_config: Dict):
        ...
        out = [
            {"params": field_params, "lr": lr_config['base']},
            {"params": nn_params, "lr": lr_config['base']},
            {"params": other_params, "lr": lr_config['base']},
        ]
        if self.field_type == 'kplane_rbf':
            out.append({"params": model_params['lc0'], "lr": lr_config['lc0']})
            out.append({"params": model_params['kc0'], "lr": lr_config['kc0']})
            out.append({"params": model_params['ks0'], "lr": lr_config['ks0']})
        return out
