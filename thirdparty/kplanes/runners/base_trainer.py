...
from thirdparty.kplanes.utils.ema import EMA
from thirdparty.kplanes.models.lowrank_model import LowrankModel
from thirdparty.kplanes.utils.my_tqdm import tqdm
from thirdparty.kplanes.ops.image import metrics
from thirdparty.kplanes.ops.image.io import write_png
from thirdparty.kplanes.runners.regularization import Regularizer
from thirdparty.kplanes.ops.lr_scheduling import (
...
class BaseTrainer(abc.ABC):
    ...
    def post_step(self, progress_bar):
        init_data = self.model.step_after_iter(self.global_step)
        if self.global_step % self.calc_metrics_every == 0:
    ...
    def train(self):
        ...
        try:
            ...
            while self.global_step < self.num_steps:
                ...
                init_data = self.post_step(progress_bar=pb)
    ...
    def evaluate_metrics(self,
        ...
        if gt is not None:
            ...
            out_img_with_gt = torch.cat((out_img, gt), dim=1)
            # out_img = torch.cat((out_img, self._normalize_err(preds_rgb, gt)), dim=0)
            out_img_with_gt = torch.cat((out_img_with_gt, 
                (preds_rgb - gt).abs().mean(-1, keepdim=True).expand(-1, -1, out_img.shape[-1])*10), dim=1).clamp(0, 1)

        out_img_np: np.ndarray = (out_img * 255.0).byte().numpy()
        out_img_with_gt_np: np.ndarray = (out_img_with_gt * 255.0).byte().numpy()
        ...
        if save_outputs:
            out_name = f"rgb-step{self.global_step}-{img_idx}"
            out_with_gt_name = f"rgb_with_gt-step{self.global_step}-{img_idx}"
            if name is not None and name != "":
                out_name += "-" + name
                out_with_gt_name += "-" + name
            write_png(os.path.join(self.log_dir, out_name + ".png"), out_img_np)
            write_png(os.path.join(self.log_dir, out_with_gt_name + ".png"), out_img_with_gt_np)
            if out_depth is not None:
                depth_name = f"depth-step{self.global_step}-{img_idx}"
                write_png(os.path.join(self.log_dir, depth_name + ".png"), out_depth_np)

        return summary, out_img_with_gt_np, out_depth_np
    ...
    def init_optim(self, **kwargs) -> torch.optim.Optimizer:
        ...
        if optim_type == 'adam':
            optim = torch.optim.Adam(params=self.model.get_params(kwargs['lr_config']), eps=1e-15)
        else:
